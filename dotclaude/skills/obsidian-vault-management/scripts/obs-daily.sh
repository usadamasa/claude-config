...
read_output=$(obsidian read path="${DAILY_PATH}" ${VAULT_OPT} 2>/dev/null)

if echo "$read_output" | grep -q "^Error:"; then
  echo "Creating daily note: ${DAILY_PATH}"
  # テンプレートがあれば使用、なければフォールバック
  template_output=$(obsidian template:read name=Daily ${VAULT_OPT} 2>/dev/null)
  if echo "$template_output" | grep -q "^Error:"; then
    obsidian create path="${DAILY_PATH}" open ${VAULT_OPT} \
      content="## DAILY PLAN\n\n## TODO\n\n## やったこと\n\n## 後で読む\n![[あとで読む]]" 2>/dev/null
  else
    obsidian create path="${DAILY_PATH}" template=Daily open ${VAULT_OPT} 2>/dev/null
  fi
else
  echo "Daily note already exists: ${DAILY_PATH}"
  obsidian open path="${DAILY_PATH}" ${VAULT_OPT} 2>/dev/null
fi
