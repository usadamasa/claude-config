...
go-arch-lint v3 形式で `canUse: []` と書くと以下のエラーが発生する:
...
| `should have ref in 'mayDependOn'/'canUse' or at least one flag` | `canUse: []` (空配列) を使っている | `anyVendorDeps: true` に変更するか deps エントリを省略 |
